...
Public oRSfp As New ADODB.Recordset
Public oFrmIns As frmInsumos
...
Public Const TituloSistema As String = "Sistema de Restaurantes"
...
Public Sub CONEXION_GEN()
  ...
  CONn$ = "dsn=" & wdsn & ";uid=sa;pwd=" & claveAcceso & ";database=bdatos;"
  ...
  Pro_Aumento 15
  Splash.lblmensa.Caption = "Identificando relación de archivos..."
  DoEvents
  ...
  Pro_Aumento 3
  pub_cadena = "SELECT * FROM PEDIDOS WHERE PED_TIPMOV = ? AND PED_CODCIA = ? AND PED_NUMSER = ? AND PED_NUMFAC = ? AND PED_ESTADO = 'N' AND PED_SITUACION <> 'A' ORDER BY  PED_NUMSEC"
...
'  Splash.rctStatusBar.Value = Splash.rctStatusBar.Value + 100
  ...
  Pro_Aumento 49
  Splash.lblmensa.Caption = "Información de archivos de diario..."
  DoEvents
...
WLV1.ColumnHeaders.Add 3, , "Dirección", 3200
...
'Public Function Mayusculas(cadena As String) As String
