...
Begin VB.Form frmARTI 
   ...
   Begin VB.Frame Fdatos 
      ...
      Begin VB.Label lblnomarti 
         AutoSize        =   -1  'True
         Caption         =   "Nombre del Plato,Insumo o Comboo"
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
...
Else 'Actualiza Datos
    If frmARTI.DS.ListIndex = -1 Then
       MsgBox "Determinar la Moneda del Insumo,Plato o Servicio...", 48, Pub_Titulo
       frmARTI.DS.SetFocus
       SendKeys "%{UP}"
       Exit Sub
    End If
...
Dim marca As String
     ...
     If Left(cmdModificar.Caption, 2) = "&G" Then
       ...
       If Op(0).Value Then
        ...
        'cnPre.Open "Provider=SQLOLEDB.1;Integrated Security=SSPI;Persist Security Info=False;User ID=sa;Initial Catalog=bdatos;Data Source=laptop"
...
If LK_EMP = "3AA" Then
  If cmddolares.Tag = "D" Then
    cmddolares.Caption = "Lista de Precios en S/. (SOLES)"
    cmddolares.Tag = "S"
  Else
...
If Left(cmdModificar.Caption, 2) = "&M" Then
    ...
    If LK_EMP_PTO = "A" Then
      If LK_CODCIA <> "00" Then
        ...
        BLOQUEA_TEXT art_situacion, DS, cheservi(0), cheservi(1), cheservi(2), cheservi(3), txtMin, txtMax
      End If
    End If
    If LK_EMP = "HER" Then
       DESBLOQUEA_TEXT txtpor1, txtpor2, txtpor3, txtpor4, txtpor5, txtpor6
...
'checambio.Value = 0
If Nulo_Valors(artloc_llave!art_descontarstock) = True Then
   checambio.Value = 1
Else
checambio.Value = 0
End If
...
ElseIf Nulo_Valors(artloc_llave!art_flag_stock) = "P" Then
 cheservi(1).Value = True
ElseIf Nulo_Valors(artloc_llave!art_flag_stock) = "C" Then
 cheservi(2).Value = True
ElseIf Nulo_Valors(artloc_llave!art_flag_stock) = "I" Then
 cheservi(3).Value = True
End If
...
IR_GRABA1:
    ...
    End If
'quitado gts para el mochica p q no actualize codigos alternos
'    If LK_FLAG_ORIGINAL = "A" Then
 ...
 '   End If
    ...
    artloc_llave!art_descontarstock = True
    Else
    artloc_llave!art_descontarstock = False
    End If
    ...
    ElseIf cheservi(2).Value = True Then
        artloc_llave!art_flag_stock = "C"
    ElseIf cheservi(3).Value = True Then
        artloc_llave!art_flag_stock = "I"
    End If
...
IR_POR_CIA: ' Actualiza Cias o Cia Actual
    ...
    If Left(cmdModificar.Caption, 2) = "&G" Then
        ...
        Do Until pre_mayor.EOF
            ...
            ''***********************************
            ''***********************************AGREGADO 31 May 2006
            pre_mayor!PRE_PRE1 = val(grid_unid.TextMatrix(fila, 5))
            pre_mayor!PRE_PRE2 = val(grid_unid.TextMatrix(fila, 7))
            pre_mayor!PRE_PRE3 = val(grid_unid.TextMatrix(fila, 9))
            pre_mayor!PRE_PRE4 = val(grid_unid.TextMatrix(fila, 11))
            pre_mayor!PRE_PRE5 = val(grid_unid.TextMatrix(fila, 13))
       
            pre_mayor!PRE_PRE6 = val(grid_unid.TextMatrix(fila, 30))
            ''***********************************PRECIOS6
...
Public Sub ElGrid_EnterCell(wsGrid As MSFlexGrid, _
                            wsTexto As TextBox, _
                            Optional Bloq1, _
                            Optional Bloq2, _
                            Optional Bloq3, _
                            Optional Bloq4, _
...
Public Sub ElGrid_KeyPress(wsGrid As MSFlexGrid, _
                           wsTexto As TextBox, _
                           wsKeyAscii, _
                           Optional SaltaCol)
...
Public Sub TEXTO_KeyDown(wsGrid As MSFlexGrid, _
                         wsTexto As TextBox, _
                         wsKeyCode As Integer, _
                         Optional SaltaCol)
                ...
                Select Case wsGrid.COL
                    ...
                    Case 5
                        ...
                        If frmARTI.cmddolares.Tag = "D" Then
                            wsGrid.TextMatrix(wsGrid.Row, 16) = Format(wsTexto.Text, "0.0000")
                        Else
                            wsGrid.TextMatrix(wsGrid.Row, 21) = Format(wsTexto.Text, "0.0000")
                        End If

                    Case 7

                        If frmARTI.cmddolares.Tag = "D" Then
                            wsGrid.TextMatrix(wsGrid.Row, 17) = Format(wsTexto.Text, "0.0000")
                        Else
                            wsGrid.TextMatrix(wsGrid.Row, 22) = Format(wsTexto.Text, "0.0000")
                        End If
...
Public Sub TEXTO_KeyPress(wsGrid As MSFlexGrid, _
                          wsTexto As TextBox, _
                          wsKeyAscii As Integer, _
                          Optional SaltaCol, _
                          Optional ConsisCol1, _
                          Optional ConsisVal1, _
                          Optional ConsisCol2, _
                          Optional ConsisVal2, _
                          Optional ConsisCol3, _
                          Optional ConsisVal3, _
                          Optional ConsisCol4, _
                          Optional ConsisVal4, _
                          Optional ConsisCol5, _
                          Optional ConsisVal5, _
                          Optional ConsisCol6, _
                          Optional ConsisVal6, _
                          Optional ConsisCol7, _
                          Optional ConsisVal7, _
                          Optional ConsisCol8, _
                          Optional ConsisVal8, _
                          Optional ConsisCol9, _
                          Optional ConsisVal9, _
                          Optional ConsisCol10, _
                          Optional ConsisVal10, _
                          Optional ConsisCol11, Optional ConsisVal11, Optional ConsisCol12, Optional ConsisVal12)
...
Private Sub Consistencias(wsGrid As MSFlexGrid, _
                          wsTexto As TextBox, _
                          wsKeyAscii As Integer, _
                          Optional ConsisVal, _
                          Optional ConsisCol)
...
Public Sub CALCULAR_PORCOL(ByVal WSPRE As Currency, _
                           ByVal WSCOL As Integer, _
                           ByVal WSROW As Integer)

    Dim valor As Currency

    If val(grid_unid.TextMatrix(WSROW, 3)) <> 0 Then
        valor = (WSPRE * 100) / val(grid_unid.TextMatrix(WSROW, 3)) - 100
    Else
        valor = 0
    End If
    ...
    grid_unid.TextMatrix(WSROW, WSCOL - 1) = Format(valor, "0.0000")
...
Private Sub grid_UNID_LeaveCell()
    ...
    If Flag_Change <> "A" Then
        ...
        If grid_unid.COL = 3 Or grid_unid.COL = 5 Or grid_unid.COL = 7 Or grid_unid.COL = 9 Or grid_unid.COL = 11 Or grid_unid.COL = 13 Or grid_unid.COL = 30 Then ' costo PORCENTAJE
            CALCULAR_POR val(grid_unid.TextMatrix(grid_unid.Row, grid_unid.COL)), grid_unid.COL
            '==================================================================================
            ''agregado para calculo automatico de precios en diferentes unidades
...
Public Function EXISTE_ART(WARTI As String, WCODI As String) As Boolean
    ...
    tempo = Left(Trim(WARTI), Len(WARTI))  'tempo = Left(Trim(WARTI), Len(WARTI)-1) aca gts cambie
...
Private Sub cmd_AddItem_Click()
    ...
    sCnnSQL = "Provider=SQLOLEDB.1;Persist Security Info=False;pwd=" & claveAcceso & ";User ID=sa;Initial Catalog=bdatos;Data Source=laptop" '"Provider=MSDASQL;Data Source=dsn_datos" '"provider=SQLOLEDB;Data source=PC01;initial catalog=bdatos;password=;user id=sa"
